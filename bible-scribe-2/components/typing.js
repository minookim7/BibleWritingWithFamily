...
export class TypingEngine {
  ...
  attach(inputEl) {
    ...
    // IME 조합 시작/끝 추적. 조합 중에도 화면은 갱신하되, 최종 채점은 조합 완료 시.
    ...
    this._onInput = (e) => {
      ...
      // 조합 중에도 실시간으로 화면 갱신(한글이 차오르는 느낌). 채점은 완료 시.
      this._handleValue();
    };
  ...
  _handleValue() {
    ...
    // 오타 집계: 조합이 끝난 확정 상태에서만, 전체를 다시 비교해 정확히 계산.
    // (증분 추적 대신 전체 비교 → 한글 조합/백스페이스에도 어긋나지 않음)
    if (!this.composing) {
      // "지금까지 도달한 최대 지점까지" 틀린 글자 수를 누적 오타로 본다.
      // 현재 입력 구간에서 목표와 다른 글자 개수를 세되, 한 번 틀린 위치를
      // 반복 집계하지 않도록 현재 상태 기준의 오타 수로 갱신.
      let wrong = 0;
      for (let i = 0; i < value.length; i++) {
        if (value[i] !== this.target[i]) wrong++;
      }
      // 최고 도달 오타를 유지(고쳐서 줄어드는 건 반영하되, 음수 방지)
      this.mistakeCount = Math.max(this._sessionWrong || 0, wrong);
      this._sessionWrong = wrong;
    }

    const state = this.computeState();
    this.onUpdate(state);

    // 완료 판정은 조합이 끝난 확정 상태에서만.
    if (!this.composing &&
        value.length === this.target.length &&
        state.correctChars === this.target.length) {
      this.finished = true;
  ...
  computeState() {
    const chars = [];
    let correct = 0;
    // 조합 중이면 마지막 입력 글자는 아직 확정 전이므로 오답 표시하지 않음.
    const composingIdx = this.composing ? this.typed.length - 1 : -1;
    for (let i = 0; i < this.target.length; i++) {
      const expected = this.target[i];
      let status = 'pending';
      let typedChar = null;                 // 이 위치에 실제로 입력된 글자(화면 표시용)
      if (i === composingIdx) {
        status = 'current';                 // 조합 중인 글자: 진행 표시
        typedChar = this.typed[i];          // 조합 중인 글자를 그대로 보여줌 (ㅌ→태→택)
      } else if (i < this.typed.length) {
        typedChar = this.typed[i];
        status = typedChar === expected ? 'correct' : 'wrong';
        if (status === 'correct') correct++;
      } else if (i === this.typed.length) {
        status = 'current';
      }
      chars.push({ char: expected, typedChar, status });
    }
