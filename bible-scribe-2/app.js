...
// (시편 119편 = 6,600자 → 전체 재생성 시 키마다 6,600 span 생성. 패치 방식은 2~3개만.)
...
function buildPassage(state) {
  ...
  _charSpans = new Array(state.chars.length);
  _charStatuses = new Array(state.chars.length).fill('');
...
function patchPassage(state) {
  // 상태가 바뀐 글자만 클래스 갱신 (보통 2~3개)
  for (let i = 0; i < state.chars.length; i++) {
    const s = state.chars[i].status;
    if (_charStatuses[i] !== s) {
      const span = _charSpans[i];
      const isSpace = state.chars[i].char === ' ';
      span.className = `ch ${s}${isSpace ? ' space' : ''}`;
      _charStatuses[i] = s;
    }
  }
}
